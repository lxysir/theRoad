...
# automation the string
# 	genetations ** times


def cellular_automaton(string, pattern_number, generations):
    pattern = {}
    origin_pattern = ['...', '..x', '.x.', '.xx', 'x..', 'x.x', 'xx.', 'xxx']
    n = len(string)
    for i in range(7, -1, -1):
        if int(pattern_number/(2**i)) == 1:
            pattern[origin_pattern[i]] = 'x'
            pattern_number = pattern_number - 2**i
        else:
            pattern[origin_pattern[i]] = '.'
    for j in range(0, generations):
        new_string = ''
        for i in range(0, n):
            autoword = string[i-1] + string[i] + string[(i+1) % n]
            new_string = new_string + pattern[autoword]
        string = new_string
    return pattern, new_string
