...
def seperate_names(user_data, include_string):
    start_position = user_data.find(include_string)
    username = user_data[:start_position]
    name_string = user_data[start_position + len(include_string):]
    name_list = name_string.split(',')
    for i in range(0, len(name_list)):
        name_list[i] = name_list[i][1:]
    return username, name_list
